...
std::string ImplLogger::getLogLevelColor(const LogLevel level) const {
    ...
    switch (level) {
        case LogLevel::DEBUG:
            result = mColored ? std::string{"\033[34m" + logLevelFormat(LogLevel::DEBUG) + "\033[0m"}
                              : logLevelFormat(LogLevel::DEBUG);
            break;
        case LogLevel::INFO:
            result = mColored ? std::string{"\033[32m" + logLevelFormat(LogLevel::INFO) + "\033[0m"}
                              : logLevelFormat(LogLevel::INFO);
            break;
        case LogLevel::WARN:
            result = mColored ? std::string{"\033[33m" + logLevelFormat(LogLevel::WARN) + "\033[0m"}
                              : logLevelFormat(LogLevel::WARN);
            break;
        case LogLevel::ERROR:
            result = mColored ? std::string{"\033[31m" + logLevelFormat(LogLevel::ERROR) + "\033[0m"}
